...
> Whenever the system adds, modifies, or deletes a Domain, Layer, or Skill, the AI Agent MUST update `.agents/00-system-rules/_index/os-registry.yaml` FIRST. This acts as the runtime self-awareness database, preventing architectural drift.
...
> After an AI Agent successfully executes any Skill, it MUST generate a standalone Markdown report summarizing the run: `docs/06-records-history/execution-summaries/YYYY-MM-DD-SKILL-[ID]-[request].md`.
...
| :--- | :--- | :--- | :--- |
| **⚙️ OS Engine** | `.agents/00-system-rules/` | The DNA. Contains Prompts, YAML Mappings, and Templates. | Base System |
| **🏭 Workflow Routing**| `.agents/02-execution-workflows/` | The actual Agentic State Machine and rule orchestrators. | AI Orchestrator |
| **📦 OS Templates** | `.agents/03-os-templates/` | The Boilerplate Seed. Deterministic workspace injected via Skill 00. | Base System |
| **📥 Pipeline Input** | `docs/01-execution-tickets/` | The Universal Backlog. Feature, Ops, and Migration tickets live here. | Human PMs & Ops |
| **📝 Human Draft Area**| `docs/03-artifacts-draft/` | Iterative drafts generated by AI and reviewed by Humans. | Human Engineers |
| **🧠 AI Knowledge** | `docs/04-knowledge-prod/` | Granular, O(1) indexed Single Source of Truth for downstream AI. | AI Encoders |
| **🧰 Support Assets** | `docs/05-support-assets/` | Mock data, static testing helpers. | Hybrid |
| **🗄️ Historical Log** | `docs/06-records-history/` | Cold storage for bugs, post-mortems, and deployments. | Reference |
...
- **Skill 01 (Execution Ticket Dispatcher):** Interactively interviews users to generate an Execution Ticket (Greenfield, Brownfield, or Ops) and routes it to the correct unit.
...
- **Skill 22 (Install Framework Plugin):** Copies the selected `code-mappings`, `code-templates`, and `scripts` from `.agents/03-os-templates/framework-plugins/` into the user's `docs/05-support-assets/`.
...
### 🚑 The SRE & Operations Unit (Production Ops)
Handles Post-Deployment runtime requirements, resolving bugs and performing manual operations.
- **Skill 29 (Incident Triage Analyzer):** Reads raw crash logs (e.g., Sentry, Bug Tickets) from `docs/01-execution-tickets/` and queries the `04-knowledge-prod/_index` to deterministically pinpoint the exact Domain and Business Rule being violated. Outputs a `HOTFIX-PLAN.md`.
- **Skill 30 (Hotfix Code Generator):** Executes the Hotfix Plan. Modifies physical source code, writes regression tests, and **STRICTLY updates the relevant Knowledge Base YAML** to ensure the System Architecture self-evolves from the exception.
- **Skill 31 (Data Operations Runner):** Generates and runs one-off data patching scripts (SQL/API) to resolve Support Tickets without building new software features.
- **Skill 32 (Infrastructure Ops Manager):** Reads DevOps requirements to autonomously tune or scale `docker-compose.yml`, Kubernetes manifests, or Terraform definitions mapped in the Strategic Architecture.
